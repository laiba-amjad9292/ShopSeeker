Map<String, String> ar = {
  // SignIn and SignUp text
  "create_account": "إنشاء حساب",
  "signup": "اشتراك",
  "signin": "تسجيل الدخول",
  "continue_without_login": "متابعة بدون تسجيل الدخول",

  "sign_in_your_account": "تسجيل الدخول \n إلى حسابك",
  "sign_in_subtitle":
      "ابحث عن المساحة المثالية لمشروعك القادم أو اجعل مشروعك مرئيًا لآلاف الأشخاص.",
  "create_your_new_account": "إنشاء حسابك الجديد",
  "sign_up_subtitle":
      "سواء كنت تملك محلاً تجارياً أو كنت تبحث عن واحد – نحن نضمن أنك ستجد تمامًا ما تحتاج إليه.",

  "full_name": "الاسم الكامل",
  "enter_full_name": "أدخل اسمك الكامل",
  "email": "بريد إلكتروني",
  "enter_email": "أدخل بريدك الإلكتروني",
  "password": "كلمة المرور",
  "enter_password": "أدخل كلمة المرور الخاصة بك",
  "confirm_password": "تأكيد كلمة المرور",
  "confirm_your_password": "تأكيد كلمة المرور الخاصة بك",

  "dont_have_an_account": " ليس لديك حساب؟ ",
  "already_have_an_account": " هل لديك حساب بالفعل؟ ",

  "selecting_term_policy": "من خلال إنشاء حساب فإنك توافق على شروطنا ",
  "terms_of_services": " شروط الخدمة ",
  "and": "و",
  "privacy_policy": " سياسة الخصوصية. ",

  // Validation errors
  "this_field_is_required": " هذه الخانة مطلوبه ",
  "enter_valid_email": " أدخل بريدًا إلكترونيًا صالحًا ",
  "password_does_not_match": " كلمة المرور غير متطابقة ",
  "email_not_found":
      "لم نتمكن من العثور على حساب باستخدام عنوان البريد الإلكتروني المقدم",
  "email_or_password_are_incorrect":
      "البريد الإلكتروني أو كلمة المرور غير صحيحة. يُرجى المحاولة مرة أخرى.",
  "no_user_found_with_provided_email":
      "لم يتم العثور على أي مستخدم لديه عنوان البريد الإلكتروني المقدم",
  'password_must_be_of_6_characters': 'يجب أن تكون كلمة المرور مكونة من ٦ أحرف',
  "error_occur": " حدث خطأ ",
  "you_need_to_agree_to_our_terms_and_condition":
      " يجب عليك الموافقة على شروطنا وأحكامنا ",
  "email_already_in_use":
      " البريد الإلكتروني قيد الاستخدام بالفعل من قبل شخص آخر ",
  "password_too_weak": " كلمة المرور ضعيفة جدًا ",

  //Forgot password text
  'forgot_password': 'هل نسيت كلمة السر؟',
  "password_recovery": "استعادة كلمة المرور",
  "forgot_password_subheading":
      "سوف نقوم بإرسال بريد إلكتروني إلى عنوان البريد الإلكتروني الذي قمت بتسجيله لاستعادة كلمة المرور الخاصة بك.",
  "reset_password": "إعادة تعيين كلمة المرور",
  "back_to_login": "العودة إلى تسجيل الدخول",
  "password_reset_link_sent":
      "إذا كان البريد الإلكتروني الذي قدمته موجودًا في قواعد بياناتنا كمستخدم صالح، فستتلقى رسالة بريد إلكتروني تحتوي على تعليمات إعادة التعيين.",
  "thanks": "شكرًا",

  // Signup success screen
  "success": "نجاح!",
  "account_created_successfully": "لقد تم إنشاء حسابك \nبنجاح.",
  "register_shop": "تسجيل المتجر",
  "find_shop": "ابحث عن متجر",

  // Navbarscreen
  "home": "بيت",
  "shops": "المحلات التجارية",
  "chats": "الدردشات",
  "notification": "إشعار",
  "profile": "حساب تعريفي",

  // Account Access Required
  "account_access_required": "مطلوب الوصول إلى الحساب",
  "account_access_required_sub-heading":
      "للمتابعة في هذا الإجراء، يجب عليك تسجيل الدخول. يرجى تسجيل الدخول إلى حسابك، أو إذا لم يكن لديك حساب، يمكنك التسجيل.",

  //Home Screen
  "hello": "مرحبًا!",
  "search": "بحث",
  "search_your_shop": "ابحث في متجرك...",
  "arab_market": "السوق العربي",

  //Confirm logout bottomsheet
  "confirm_logout_bottomsheet_title": "تأكيد تسجيل الخروج؟",
  "confirm_logout_bottomsheet_subtitle":
      "هل أنت متأكد أنك تريد تسجيل الخروج من حسابك؟",
  "confirm_logout_bottomsheet_yes_button": "تأكيد تسجيل الخروج",
  "confirm_logout_bottomsheet_cancel_button": "إلغاء",

  //Profile Text
  "logout": "تسجيل الخروج",

  //Add Update Screen
  "Add_Listing": "إضافة إعلان",
  "add_shop": "إضافة متجر",
  "shop_name": "اسم المتجر",
  "enter_shop_name": "أدخل اسم المتجر",
  "category": "الفئة",
  "enter_shop_category": "أدخل فئة المتجر",
  "street_and_house_no.": "الشارع ورقم المنزل",
  "enter_street_and_house_no.": "مثال: شارع النصر ١٢، منزل ٤ب.",
  "country": "البلد",
  "enter_country": "أدخل البلد",
  "city": "المدينة",
  "enter_city": "أدخل المدينة",
  "postal_code": "الرمز البريدي",
  "enter_postal_code": "أدخل الرمز البريدي",
  "timing_weekdays": "أوقات العمل (أيام الأسبوع)",
  "timing_weekends": "أوقات العمل (عطلة نهاية الأسبوع)",
  "opening_time": "وقت الفتح",
  "select_opening_time": "اختر وقت الفتح",
  "closing_time": "وقت الإغلاق",
  "select_closing_time": "اختر وقت الإغلاق",
  "description": "الوصف",
  "write_here": "اكتب هنا...",
  "Add_images": "إضافة صور",

  // Attention Needed
  "Attention_Needed": "تنبيه هام",
  "Attention_data_lost": "إذا عدت للخلف، فسيتم فقدان بياناتك.",
  "go_back": "العودة",
  "cancel": "إلغاء",

  //Confirm Delete Listing
  "Confirm_Delete": "تأكيد الحذف",
  "Confirm_Delete_subheading": "اضغط على تأكيد الحذف لحذف الإعلان",
  "Delete": "حذف",

  //Delete Media Preview
  "do_you_want_to_Delete": "هل تريد حذف الصورة؟",
  "do_you_want_to_Delete_subheading":
      "أكد إذا كنت تريد حذف الصورة من الإعلان، هذا الإجراء لا يمكن التراجع عنه.",

  // Global texts
  "session_expired_login_again":
      "انتهت صلاحية الجلسة. يرجى تسجيل الدخول مرة أخرى.",
  "something_went_wrong_try_again": "حدث خطأ ما! يرجى المحاولة مرة أخرى.",
  "check_your_connection": "يرجى التحقق من اتصال الإنترنت الخاص بك.",
  "Please_allow": "يرجى السماح",
  "permission_from_app_settings": "بالوصول من إعدادات التطبيق",
  "Open_Settings": "فتح الإعدادات",
  "Search_Country_Code": "ابحث عن رمز الدولة",
  "select_time_heading": "اختر الوقت",
  "select_time_ok_button_title": "موافق",
  "select_time_cancel_button_title": "إلغاء",

  "gallery": "المعرض",
  "camera": "الكاميرا",
  "video": "فيديو",
  "image": "صورة",
  "audio": "صوت",
  "Profile": "الملف الشخصي",

  //shop details
  "Listing_Details": "تفاصيل إعلان المتجر",
  "Edit_Shop": "تعديل المتجر",
  "Category": "الفئة: ",
  "address": "العنوان",
  "timing_on_weekday": "مواعيد العمل في أيام الأسبوع: ",
  "timing_on_weekend": "مواعيد العمل في عطلة نهاية الأسبوع: ",
  "products": "المنتجات",

  "No_shops_found": "لم يتم العثور على متاجر",
  "Not_found": "غير موجود",
  "Stay_tuned_for_updates": "تابعنا للحصول على التحديثات",
  "No_Listings_Found": "لم يتم العثور على أي قوائم",
  "please_add_a_new_shop_to_get_started": "يرجى إضافة متجر جديد للبدء",
};
